...
![Gupshup](https://img.shields.io/badge/Gupshup_CPaaS-6C63FF?style=for-the-badge&logoColor=white)
...
|---------|-------------|------------|
| **Global SMPP Messaging Platform** | Enterprise SMS platform integrated with **680+ international carriers** for high-volume, cost-efficient message delivery with real-time analytics | `Spring Boot` `Kubernetes` `SMPP` `Redis` `RabbitMQ` `Hazelcast` |
| **CPaaS Messaging Platform** | Cloud communication platform with **WhatsApp Business API** via Gupshup — template management, bulk campaigns, real-time delivery tracking | `Spring Boot` `Microservices` `WhatsApp API` `Docker` |
| **Antibody Registration Portal** | Web platform for pharmaceutical researchers to register, track, and manage antibody/DNA/ADC compounds with audit trails and SSO | `Spring Boot` `MySQL` `Redis` |
| **Local Bypass Routing System** | Intelligent SMS routing system with spam filtering and dynamic routing logic to reduce delivery costs | `Java 17` `Spring Boot` `Kubernetes` `SMPP` `Hazelcast` |
| **RemoteVal** | Remote property inspection platform with live video evaluations using Twilio and async processing via RabbitMQ | `Java 11` `Spring Boot` `Twilio` `RabbitMQ` `Azure` |
| **JobCart** | Service job tracking system for technician workflow management with admin dashboards | `Spring Boot` `Hibernate` `MySQL` `JSP` |
...
I'm always open to discussing **system architecture**, **messaging platforms**, or **backend engineering** challenges.
